...
fn main() {
    let cfg: MyConfigs = confy::load("read_for_me", None).unwrap();
    ...
    for file in all_files{
        ...
        let tag = Tag_::new().read_from_path(file.as_os_str());
        match tag {
            Ok(audiotags) => {
    ...
    let source = {        
        Box::new(File::open(&atlas_shrugged).unwrap())};
    ...
    match symphonia::default::get_probe().format(&hint, mss, &format_opts, &metadata_opts) {
        Ok(mut probed) => {
            ...
            {
                // Playback mode.
                print_format(&atlas_shrugged, &mut probed);
                ...
                // Play it!
                play(probed.format, track, Some(0.0), &decode_opts, no_progress)
            };
